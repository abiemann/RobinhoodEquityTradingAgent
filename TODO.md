...
- **The deterministic layer stays sacred** (per AGENTS.md): every quantifiable criterion in a SKILL.md (cup depth %, handle drift bounds, dip %) also ships as a per-skill checker script (`skills/<name>/check.py`) with tests in `tests/`. Prose explains judgment; scripts compute numbers; vision breaks ties.
