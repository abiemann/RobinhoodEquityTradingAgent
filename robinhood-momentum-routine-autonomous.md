...
where `rhmra` abbreviates this document's name (fixed literal — do not expand it), and `YYYY_MM_DD`-`HH_MM` is the local date and time AT WHICH THE RUN STARTED executing this document (year first so filenames sort chronologically; zero-padded, 24-hour clock). Check the clock as one of your first actions and hold onto that value: the filename must carry the run's start time, not the time the report is finally written — a long run must not drift to its finish time. E.g. a run that begins on 2 July 2026 at 1:05 PM saves as `rhmra-log-2026_07_02-13_05.md` even if the report is written at 1:12 PM. Do not save the report anywhere else, do not invent a different filename pattern, and do not overwrite or append to a previous run's file.
