...
2. If a position is up `TAKE_PROFIT_PCT` or more vs. entry: sell the entire position at market (`place_equity_order`, market sell) AND cancel any open stop-loss order tied to it — find it via `get_equity_orders` and cancel with `cancel_equity_order`. Fire the trade notification for the sell.
   ...
   **Stop-coverage audit (same step, every run):** stops disappear silently — a gfd stop expires at that day's close, and an extended-hours fill never gets its stop because each run is a fresh session (this is how a position once rode to −49% unprotected). So: for every position still held after profit-taking, with at least 1 share and market value ≥ $10, check `get_equity_orders` for an open stop-loss sell. If none exists, place one — GTC, whole shares, stop price = `STOP_LOSS_PCT` below the CURRENT price (current, not cost basis: a below-cost stop on an already-underwater position would trigger immediately as a market sell). Fire the trade notification and list it in the report as a stop repair. Positions under 1 share or $10 cannot carry a stop — report them as unprotected dust instead.
...
**Buying-power gate (check before Step 8):** you already know buying_power (`get_portfolio`, FIRST). The EFFECTIVE order size this run = min(`BUY_SIZE_PCT`/100 × total_value, buying_power rounded down to the dollar). If the effective size is below `MIN_ORDER_DOLLARS`, no meaningful buy can be placed — skip Steps 8–12 entirely (fetch no historicals, run no script) and report the working list from THIRD with the note "entry evaluation skipped: effective order size $X < MIN_ORDER_DOLLARS — cash settling, see Tradeoffs". Otherwise proceed: a downsized order is still an order.
