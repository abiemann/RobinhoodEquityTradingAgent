...
2. If a position is up `TAKE_PROFIT_PCT` or more vs. entry: sell the entire position at market (`place_equity_order`, market sell) AND cancel any open stop-loss order tied to it — find it via `get_equity_orders` and cancel with `cancel_equity_order`. Fire the trade notification for the sell.
   ...
   **Stop-coverage audit (same step, every run):** stops disappear silently — a gfd stop expires at that day's close, and an extended-hours fill never gets its stop because each run is a fresh session (this is how a position once rode to −49% unprotected). So: for every position still held after profit-taking, with at least 1 share and market value ≥ $10, check `get_equity_orders` for an open stop-loss sell. If none exists, place one — GTC, whole shares, stop price = `STOP_LOSS_PCT` below the CURRENT price (current, not cost basis: a below-cost stop on an already-underwater position would trigger immediately as a market sell). Fire the trade notification and list it in the report as a stop repair. Positions under 1 share or $10 cannot carry a stop — they are handled by the dust sweep below.

   **Dust sweep (same step — regular hours only):** any STANDALONE position totaling less than 1 share is unprotectable dust — typically the fractional tail orphaned when a whole-share stop fired (possibly late the prior day). During regular market hours (09:30–16:00 ET), close each one with a fractional market sell for its full quantity (`place_equity_order`, market, `regular_hours`), through the usual `review_equity_order` check, and fire the trade notification (reason: dust sweep). Outside regular hours do NOT attempt it — fractional market sells are rejected there; report the dust as "pending sweep at next regular-hours run", which is what cleans overnight stop-out tails the next trading day. Do NOT touch a fractional tail still attached to a position of ≥ 1 whole share — that exits with its position. Swept proceeds are cash-account funds and settle T+1.
