...
8. For each ticker in the WORKING LIST: pull daily bars via `get_equity_historicals` covering at least `VOLUME_LOOKBACK_DAYS` trading days (request a wide enough range, e.g. ~30 calendar days). From these bars:
   ...
   - **Do the math with the checked-in script — never re-implement it:** save each raw `get_equity_historicals` response to a JSON file UNMODIFIED (no hand-transcribing bars), build a small JSON map of ticker → current price from the quotes call, then run `evaluate_candidates.py` (sits next to this document):
