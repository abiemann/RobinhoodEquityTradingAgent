...
The immediately following lifecycle-start cell must load `BOOTSTRAP_KEY`, require this exact `launcher-bound` shape, and construct its command from `bootstrap.resolver_receipt.python`; it may not accept a displayed resolver receipt or a second launcher. A missing reload is a failed launcher bind, not permission to continue.
