...
- **Cash-account settlement starves next-day buying power.** Sale proceeds in a cash account settle T+1, so the day after stop-outs or profit-taking, buying power can sit far below `BUY_SIZE_PCT` × total_value and the routine will skip otherwise-valid entries ("insufficient buying power"). This is by design — it skips rather than silently downsizing. If entries are skipped chronically, the deliberate options are: accept the slower cadence, lower `BUY_SIZE_PCT`, or amend Step 11 to cap the order at available buying power.
...
```
Regular-hours fractional market buy — same shape but `"type": "market"`, `"market_hours": "regular_hours"`, and replace `quantity`/`limit_price` with `"dollar_based_amount": "<BUY_SIZE_PCT × total_value>"`. Stop-loss sell — `"side": "sell"`, `"type": "stop"`, `"quantity": "<whole shares — stops do not accept fractions; floor the position>"`, `"stop_price": "<price>"`, `"market_hours": "regular_hours"`, `"time_in_force": "gtc"`. ALWAYS pass `"gtc"` explicitly on stops: the default is `"gfd"`, which cancels the stop at that day's close and leaves the position unprotected from the next session on. Confirm the market-order and stop field names on the first regular-hours run; only the extended-hours limit path is verified so far.
...
**Buying-power gate (check before Step 8):** you already know buying_power (`get_portfolio`, FIRST) and the per-order size (`BUY_SIZE_PCT`/100 × total_value). If buying_power < the per-order size, NO buy can be placed this run — Step 11 would skip every candidate (verified live: `review_equity_order` returns `EQUITY_NOT_ENOUGH_BP_DOLLAR_BASED`). Skip Steps 8–12 entirely: fetch no historicals, run no script. Report the working list (already computed in THIRD) with the note "entry evaluation skipped: buying power $X < per-order size $Y — cash settling, see Tradeoffs". On settlement-starved days this halves the run's token cost.
