...
8. For each ticker in the WORKING LIST: pull daily bars via `get_equity_historicals` covering at least `VOLUME_LOOKBACK_DAYS` trading days (request a wide enough range, e.g. ~30 calendar days). From these bars:
   ...
   - **Do the math with the checked-in script — never re-implement it:** save each raw `get_equity_historicals` response to a JSON file UNMODIFIED (no hand-transcribing bars) and build a small JSON map of ticker → current price from the quotes call. ALL scratch files (historicals JSONs, the quotes map, any `--json-out`) go in a `tmp_eval/` folder next to this document — never loose in the repo root; the folder is deleted before the report (see REPORT). Then run `evaluate_candidates.py` (sits next to this document):
...
### REPORT
**First, clean up scratch:** delete the `tmp_eval/` folder and any stray `tmp_*` files sitting next to this document — including leftovers from an earlier crashed run. Extract whatever numbers you still need from them BEFORE deleting. Use `rm -rf` — it is a no-op on missing files, so no existence check is needed. In the Cowork sandbox the project folder refuses deletions by default: if `rm` fails with `Operation not permitted`, call the `allow_cowork_file_delete` tool for this project folder (load it via tool search if needed), then retry the `rm` ONCE — do not detour into sudo, truncate, or permission spelunking. If deletion is still blocked, note it in the report and move on: cleanup must never block or delay the report. Nothing temporary outlives the run; only `run-reports/` persists. Then produce the report:
