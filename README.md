...
- **Deterministic broker-snapshot staging**: before any broker call, `broker_snapshot.py` proves the session scratch directory with a write/read/strict-parse sentinel. Every daily-loss response is then taken from the harness result file (or one untouched file-tool fallback), stripped only of a known transport envelope, semantically validated, atomically persisted, read back, and hash-checked. Paginated sets are sealed with their complete cursor chain. A failed generation is discarded wholesale and rebuilt once from fresh broker calls; files are never repaired or mixed across attempts.
...
- **Per-run status snapshot** (`run-reports/rhmra-status-*.json`, local/gitignored): raw account state after each successful final refresh — positions with purchase price, current price, and stop coverage; buying power; realized P&L; which gate decided the entry phase. Raw facts only; consumers (see `dashboard/`) do their own arithmetic. Pre-broker exits, lease loss, and failed final refresh deliberately leave the prior truthful account snapshot untouched; their lifecycle record closes the former telemetry gap.
