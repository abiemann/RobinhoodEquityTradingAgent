...
- **Deterministic status publication** (`status_snapshot.py`): the agent writes only a candidate in its marked session scratch directory. The helper requires the active lifecycle invocation, exact unrounded run start, and expected output name; rejects duplicate or non-finite JSON values, extra/missing keys, wrong types/enums, and inconsistent fields; stages and reads back the accepted bytes; and publishes atomically without overwriting an existing snapshot. A lost publish receipt is reconciled with the invocation-bound read-only verifier before any rewrite. One proven-missing candidate may be rewritten and retried once. A second failure leaves prior truthful snapshots untouched and closes lifecycle as `final-status-unavailable` / `status-write-failed`.
...
- **resolve_python.ps1** — the Windows scheduled-run bootstrap. It emits one strict JSON receipt containing a launch-probed absolute Python 3 path, rejects Microsoft Store aliases, discovers the current Codex-bundled runtime and normal local installations without hardcoding a version, and lets the routine reuse one interpreter instead of trusting whichever `python.exe` appears first on PATH. The receipt's `python` field is immediately authoritative and directly executable; `-PreferredPath` is only a discovery hint, never a value to reconcile after success.

- **status_snapshot.py** — the final account-status schema and publication authority. `publish` accepts one direct-child candidate from the marked session scratch directory only for the named active lifecycle invocation, exact bound run-start timestamp, and expected output filename; it enforces the exact version-1 document contract and cross-field semantics, then uses flushed staging, strict read-back, hashing, and atomic no-clobber publication under `run-reports/`. Its invocation-bound `verify` reconciles a lost receipt before any retry, while its read-only document validator is shared by dashboard ingestion so producer and consumer cannot silently drift to different schemas.
