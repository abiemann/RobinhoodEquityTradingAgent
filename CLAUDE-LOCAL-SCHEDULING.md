...
4. While Part A still has **Schedule: Manual**, open it and click **Run now**. Confirm native PowerShell and the resolver-bound Windows Python, one successful `get_accounts`, one exact full-object zero-prefix JSON save (no BOM/fence/label or retry), a successful `bind-transport --account-name` receipt that validates `agentic_allowed` and supplies account scope without a second lookup, lifecycle classification `completed`, a report and status snapshot, lease release, and a healthy dashboard.
